...
for (def row = 1; row <= findTestData('오늘의영어봇/원예나의마패영').getRowNumbers(); row++) {
    def utterance = findTestObject('오늘의영어봇/Shadow api(헤카앱)', [('text') : findTestData('오늘의영어봇/원예나의마패영').getValue(1, row)]) //api post

    def recognizer = findTestData('오늘의영어봇/원예나의마패영').getValue(1, row) //utterance expected

    def expected = findTestData('오늘의영어봇/원예나의마패영').getValue(3, row) //speech expected

    def intentName_expected = findTestData('오늘의영어봇/원예나의마패영').getValue(2, row)
