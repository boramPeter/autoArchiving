...
for (def row = 1; row <= findTestData('코멕스봇/공용부봇').getRowNumbers(); row++) {
    def utterance = findTestObject('코멕스봇/Shadow api', [('text') : findTestData('코멕스봇/공용부봇').getValue(1, row)])

	def recognizer = findTestData('코멕스봇/공용부봇').getValue(1, row)
	
    def expected = findTestData('코멕스봇/공용부봇').getValue(2, row)

    def intentName_expected = findTestData('코멕스봇/공용부봇').getValue(3, row)
