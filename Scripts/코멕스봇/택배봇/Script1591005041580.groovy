...
for (def row = 1; row <= findTestData('택배봇').getRowNumbers(); row++) {
    def utterance = findTestObject('코멕스봇/Shadow api', [('text') : findTestData('택배봇').getValue(1, row)])

	def recognizer = findTestData('택배봇').getValue(1, row)
	
    def expected = findTestData('택배봇').getValue(2, row)

    def intentName_expected = findTestData('택배봇').getValue(3, row)
