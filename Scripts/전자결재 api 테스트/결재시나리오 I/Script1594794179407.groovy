...
//우회로그인 
def LoginA = findTestObject('전자결재 api/우회로그인', [('아이디') : 'sunny.sim']) //내부기안자
...
RequestObject orgdraft = findTestObject('전자결재 api/결재프로세스/기안작성/기안작성(기안1부서협조1결재1)', [('결재선유형') : 'DD', ('결재타입2') : 'AP', ('결재상태2') : 'AW'])
...
//부서협조 담당자 접수 전 결재(승인,반려 = approval1)버튼, 상위결재선(upApprLine)버튼, 공람설정(share)버튼, 접수(receipt), 인쇄(print)버튼 존재 확인

RequestObject btndraftA = findTestObject('전자결재 api/결재프로세스/조회 시리즈/문서기본정보 조회',[('문서번호') : orgdocid, ('문서함') : 'EH'])

btndraftA.setHttpHeaderProperties(HTTPHeaderA)

WS.sendRequest(btndraftA)

responseA = WS.sendRequestAndVerify(btndraftA)

'결재 버튼 변수지정'
def approvalA = WS.getElementPropertyValue(responseA, getBtn1)

'상위결재선 버튼 변수지정'
def upApprLineA = WS.getElementPropertyValue(responseA, getBtn2)

'접수 버튼 변수지정'
def receiptA = WS.getElementPropertyValue(responseA, getBtn3)

'공람설정 버튼 변수지정'
def shareA = WS.getElementPropertyValue(responseA, getBtn4)

'인쇄버튼 변수지정'
def printA = WS.getElementPropertyValue(responseA, getBtn5)

WS.verifyResponseStatusCode(responseA, 200)

WS.verifyMatch(approvalA, "approval1", true, FailureHandling.CONTINUE_ON_FAILURE)

WS.verifyMatch(upApprLineA, "upApprLine", true, FailureHandling.CONTINUE_ON_FAILURE)

WS.verifyMatch(shareA, "share", true, FailureHandling.CONTINUE_ON_FAILURE)

WS.verifyMatch(receiptA, "receipt", true, FailureHandling.CONTINUE_ON_FAILURE)

WS.verifyMatch(printA, "print", true, FailureHandling.CONTINUE_ON_FAILURE)

//내부결재 기안작성
RequestObject draft = findTestObject('전자결재 api/결재프로세스/기안작성/내부결재 기안작성(기안1결재3)', [('원문서번호') : orgdocid, ('결재선유형') : 'HD', ('문서타입') : 'DR',('결재타입1') : 'AP', ('결재상태1') : 'AW'
		, ('결재타입2') : 'AP', ('결재상태2') : 'AW'])
...
//결재전 내부결재 1차결재자B 결재(승인,반려 = approval1)버튼, 상위결재선 변경 버튼(upApprLine),공람설정(share)버튼, 인쇄(print)버튼 존재 확인
...
'상위결재선 버튼 변수지정'
def upApprLineB = WS.getElementPropertyValue(responseA, getBtn2)
...
'결재 버튼 변수지정'
def approval1C = WS.getElementPropertyValue(responseCCC, getBtn1)
...
WS.verifyMatch(approval1C, "approval2", true, FailureHandling.CONTINUE_ON_FAILURE)
...
'결재선번호변수 지정'
def orgappline1E = WS.getElementPropertyValue(responseEEEE, Getappline2,FailureHandling.CONTINUE_ON_FAILURE)
