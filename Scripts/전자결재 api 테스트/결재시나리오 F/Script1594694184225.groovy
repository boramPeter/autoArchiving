...
//1차협조자 상위결재선변경버튼, 결재(승인,반려 = approval1)버튼,공람설정(share)버튼, 인쇄(print)버튼 존재 확인
'펑션에서 버튼 가져오기'
...
//협조자B 결재함 > 진행중 문서 존재 확인
'결재중의 문서 번호,아이디넘버 확인 변수 지정'
def awGetdocid1 = 'result.contents[0].docId'
def awGetid1 = 'result.contents[0].draftAccountId'

'1차협조자 인증토큰 발급'
RequestObject awingdraft = findTestObject('Object Repository/전자결재 api/결재프로세스/결재함/진행중 문서 조회',[('결재구분') : 'HE'])
...
'결재중의 기안자 변수지정'
def awingid = WS.getElementPropertyValue(responseBBBBBB, awGetid1)
...
//1차협조자 결재취소(cancel) 결재의견(changeOpinion), 인쇄버튼 존재 확인
...
//협조자B 완료된에 문서 확인 <<< 시나리오 확인중
'완료된에 기안 확인'
RequestObject fndraftB = findTestObject('Object Repository/전자결재 api/결재프로세스/결재함/완료된 문서 조회',[('결재구분') : 'HE'])

fndraftB.setHttpHeaderProperties(HTTPHeaderB)

WS.sendRequest(fndraftB)

responseBfn = WS.sendRequestAndVerify(fndraftB)

'완료된에 있는 기안 문서번호 변수지정'
def fndocidB = WS.getElementPropertyValue(responseBfn, ssgetdocid)

WS.verifyResponseStatusCode(responseBfn, 200)

WS.verifyEqual(fndocidB, docid, FailureHandling.CONTINUE_ON_FAILURE)


//협조자B 공람버튼, 인쇄버튼 존재 확인

RequestObject btndraftBBB = findTestObject('전자결재 api/결재프로세스/조회 시리즈/문서기본정보 조회',[('문서번호') : docid, ('문서함') : 'AC'])
