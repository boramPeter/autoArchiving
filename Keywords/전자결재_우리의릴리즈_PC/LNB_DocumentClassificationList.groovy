...
public class LNB_DocumentClassificationList {
	...
	@Keyword
	public void DocumentClassificationList(){
		...
		//문서대장 선택
		WebElement docClassMenu = driver.findElement(By.xpath('//*[@id="gnbContent"]/div/ul/li[7]/a/span'))
		action.click(docClassMenu)
		action.perform()

		//기안양식 셀렉트박스 선택 후 검색되는지 케이스

		draftTemplateTitle :for (int i = 40; i <= 70; i++) {
		...
		def expectedURL = "https://approval.we.kakaowork.com/box/le"
		...
		//검색필터 초기화 용도
		WebUI.navigateToUrl('https://approval.we.kakaowork.com/box/le')
		...
		if(currentURL == expectedURL)
		{
			KeywordLogger logBtn1 = new KeywordLogger()

			logBtn1.logInfo('url이 그대로임 조회버튼 기능확인 필요')
			throw new StepErrorException("url이 그대로임 조회버튼 기능확인 필요")
